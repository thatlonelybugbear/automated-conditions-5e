...
- Added `itemOptions` support to the `hasItems` filter in `checkNearby()` and `findNearby()`, allowing nearby items to be filtered by options such as item type, equipped state, and partial name matching.
  - Shield Bearer example:
    ...
    - Type: `AC5e`
    - Value: `min(3, checkNearby(effectActor.tokenId, 'same', 5, {}).filter(token => token.actor?.system?.attributes?.ac?.equippedShield).length); checkNearby(effectActor.tokenId, 'same', 5, {}).some(token => token.actor?.system?.attributes?.ac?.equippedShield)`
