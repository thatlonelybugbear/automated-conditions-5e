...
export function autoRanged(activity, token, target, options = {}) {
	...
	const item = activity?.item ?? (typeof options.item === 'string' ? fromUuidSync(options.item) : options?.item instanceof Item ? options.item : null);
	if (!token) return {};
...
export function canSee(source, target, status) {
	...
	if (status === 'blinded' || sourceBlinded) {
		validModes = ['blindsight'];
	} else if (crossPlanarEthereal) {
		validModes = ['seeAll'];
	} else if (status === 'invisible' || targetInvisible) {
		validModes = ['blindsight', 'seeAll', 'seeInvisibility'];
	}
