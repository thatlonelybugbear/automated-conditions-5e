...
`ac5e.getItem(source, itemIdentifier, options = {})` returns the first matching actor item, while `ac5e.getItems(...)` returns all matches and `ac5e.hasItem(...)` returns a boolean. `source` can be an actor, token, token ID, token UUID, actor ID, or actor UUID.
